...
def sredeem(request):
    return render(request,'sredeem.html')

def svalidatecode(request):
    if request.method == 'POST':
        code = request.POST.get('code')
        gift = giftcard.objects.get(giftcode=code)
        validity = gift.validity
        status = gift.status
        d2 = date.today()
        if validity >= d2 and status == 'new':
            gift.status = 'expiered'
            return render(request,'sredeem.html',{'msg':'redeemed'})
        if status == 'expiered':
            return render(request,'sredeem.html',{'msg':'expiered'})