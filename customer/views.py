...
def mygiftcards(request):
    return render(request,'mygiftcards.html')

def cgiftcardsave(request):
    if request.method == 'POST':
        gift = giftcard()
        cus = customerlogin.objects.last()
        custid = cus.customerid
        ...
        custrow = customers.objects.get(username=username)
        customerId = custrow.id
        gift.customer = request.POST.get('customer')
        gift.amount = request.POST.get('amount')
        d2 = date.today()
        validity = d2 + timedelta(30)
        gift.validity = validity
        gift.customerid = customerId
        code = random_with_N_digits(8)
        gift.giftcode = code
        gift.status = 'new'
        gift.save()
        return render(request,'paymentcard.html')
...
def ccardview(request):
    ...
    for i in gift:
        if i.customerid == customerId:
            lst.append(i)
    return render(request,'ccardview.html',{'gift':lst})